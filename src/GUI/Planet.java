...
public class Planet {
	...
	public Planet(int r, int g, int b, double xCoordinate, double yCoordinate, double diameter, double vx, double vy,
			double mass) {
		...
		// physical property of a planet that is different compared to x,y and d in
		// units
		this.mass = mass;
	}
	...
	public boolean collision(int x, int y, double scale) {
		if (x > 600 + (getX() - getDiameter() - 600) * scale && x < 600 + (getX() + getDiameter() - 600) * scale
				&& y > 400 + (getY() - getDiameter() - 400) * scale
				&& y < 400 + (getY() + getDiameter() - 400) * scale) {
			return true;
		} else {
			return false;
	...
	public void updatePlanet(double xNew, double yNew, double massNew) { // this
		...
		*/
		distance = Math.sqrt( (xNew - this.x)*(xNew - this.x) + (yNew - this.y)*(yNew - this.y));
		...
		*/
		...
		//check operations
		a = massNew / distance / distance;
		dirX = (xNew - this.x) / distance;
		dirY = (yNew - this.x) / distance;
